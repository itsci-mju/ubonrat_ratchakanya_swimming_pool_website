...
<head>
<meta charset="UTF-8">
<title>ViewRegisterCourse</title>
</head>
<!-- CSS only -->
...
<% 	
	...
	List<Trainees> tra = rg.SELECT_FUSION(mid);
...
%>
	...
	<%	if(tra.get(i).getRegister_courses().getLearn_time().equals("1")) {
		...
		<% if(tra.get(i).getRegister_courses().getStatus() == 0) {
			  status = "รอการชำระเงิน";
		   }else if(tra.get(i).getRegister_courses().getStatus() == 1) {
			  status = "รอการตรวจสอบยอดเงิน";
		   }else if(tra.get(i).getRegister_courses().getStatus() == 2) {
			  status = "ชำระเงินสำเร็จ";
		   }else if(tra.get(i).getRegister_courses().getStatus() == 3) {
			  status = "การชำระเงินล้มเหลว";
		}%>
		

	<form name="frm" method="post" action="ViewRegisterCourse" class="fit">
            <div align="center" style="padding-top:25px;" >
            <p style="font-size: 34px; margin: 5;" align="center" class="title">ดูแบบฟอร์มสมัครเรียนว่ายน้ำ </p>
            <table>   
            <tr>
            	<td>
             	...
             	<td>
             		<div class="form-floating">
                            <input type="text" name="status" id="status" class="form-control registered" 
                            
                            style="color:  
                            
                            <% if(status.equals("รอการชำระเงิน")) { %> red 
                            <% }else if (status.equals("รอการตรวจสอบยอดเงิน")) { %> blue 
                            <% }else if (status.equals("ชำระเงินสำเร็จ")){  %> green 
                            <% }else if (status.equals("การชำระเงินล้มเหลว")){  %> purple
                            
                            <% }%>;"
                            
                              
                            value="<%= status %>" readonly>
                            <label for="floating"><b>สถานะ</b></label>
                        </div>
             		</td>
             	</tr>   
                <tr>
                ...
                <tr>
             	 ...
             	 <a href="promptpay?cid=<%= tra.get(i).getRegister_courses().getRegister_courses_id() %>"  class="btn btn-danger" role="button">ชำระเงิน</a>
