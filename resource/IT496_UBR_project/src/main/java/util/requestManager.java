...
public class requestManager {
	...
	public List<pool_reservations> getListRequestUsePool(){
		...
		try {
			...
			String sql = "SELECT  pool_reservations_id,event_name, start_time, end_time, detail, price, document, status, members_id FROM pool_reservations where status = 0 ";
	...
	// record_usage ------------------->
	public int recordUsageService(pool_usage pus){
		ConnectionDB condb = new ConnectionDB();
		Connection con = condb.getConnection();
		try {
	        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			...
			String column = "pool_usage_id,	time, usage_type, coupon_no, amount, price, members_id";
			String sql = "insert into "+table+"("+column+") "
						+ "values('"+pus.getPool_usage_id()+"','"
